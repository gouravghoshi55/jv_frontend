...
const STATUS_OPTIONS = [
  ...
  { value: "COLD LEADS", label: "Move to Cold Leads" },
];
...
export default function NotQualifiedPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["not-qualified"],
    queryFn: () => api.get("/not-qualified/list").then((r) => r.data),
  });
  ...
  const handleUpdate = async ({ enqNo, status, remarks }) => {
    setUpdating(true);
    try {
      const res = await api.post("/not-qualified/update", { enqNo, status, remarks });
    ...
    <div>
      <div className="page-header">
        <h2 className="page-title">
          <i className="bi bi-x-circle" style={{ marginRight: 10, color: "var(--accent-red)" }}></i>
          Not Qualified Leads
        </h2>
        <span className="badge badge-red">{filteredLeads.length} leads</span>
      </div>
      ...
      <LeadTable
        ...
        actionLabel="Update"
        emptyMessage="No leads in Not Qualified."
      />
